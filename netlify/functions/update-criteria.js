...
exports.handler = async (event) => {
  try {
    ...
    if (!version) return json(400, { ok: false, error: 'Missing version' });
    if (!Array.isArray(criteria) || criteria.length === 0) return json(400, { ok: false, error: 'Missing criteria array' });

    // Basic sanity check: each row should have at least an id
    const bad = criteria.find((c) => !c || typeof c !== 'object' || !String(c.id || c.criteria_id || '').trim());
    if (bad) return json(400, { ok: false, error: 'Invalid criteria payload (missing id)' });
    ...
    return json(200, { ok: true, version, count: criteria.length });
  } catch (e) {
